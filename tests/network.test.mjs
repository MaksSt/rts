...
test('WebSocket: список комнат, 4 игрока, команды, переподключение, истечение резерва и повторный матч',async()=>{
  ...
  try{await new Promise((resolve,reject)=>{const t=setTimeout(()=>reject(Error('Server timeout')),5000);child.stdout.once('data',()=>{clearTimeout(t);resolve();});child.once('error',reject);});
    ...
    a.send({type:'command',command:{type:'build',building:'generator',x:-62,z:76}});
    const built=await b.wait(m=>m.type==='state'&&m.state.buildings.some(b=>b.owner===aid));assert.equal(built.state.buildings.find(b=>b.owner===aid).type,'generator');
    a.send({type:'command',command:{type:'upgrade',upgrade:'weapons'}});
    await b.wait(m=>m.type==='state'&&m.state.players.find(p=>p.id===aid).research?.type==='weapons');
