...
const server=http.createServer(async(req,res)=>{
  ...
  if(url.pathname==='/health'){res.setHeader('Content-Type','application/json');res.setHeader('Cache-Control','no-store');res.end(JSON.stringify({ok:true,version:'0.2.0',rooms:rooms.size,connections:wss.clients.size}));return;}
...
const port=Number(process.env.PORT||5173),host=process.env.HOST||'0.0.0.0';server.listen(port,host,()=>console.log(`IRON DUNES 0.2 — ${host}:${port} (${production?'production':'development'}), /health, request logs: ${debug?'on':'off'}`));
